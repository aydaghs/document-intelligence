...
def main() -> None:
    ...
    st.caption("OCR · Layout Analysis · Entity Extraction · Semantic Search · Summarization · Chat Q&A")
    ...
    if _api_key:
        st.sidebar.success("Claude API: connected", icon="✅")
    else:
        st.sidebar.info("Set ANTHROPIC_API_KEY for AI summarization & Q&A", icon="ℹ️")
    ...
    if use_semantic_search and search is None:
        ...
        st.sidebar.caption(f"Search backend: **{search.backend}**")

    _sidebar_api_key = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    if _sidebar_api_key:
        st.sidebar.success("Claude API key set — AI summarization & Q&A enabled")
    else:
        st.sidebar.info("No ANTHROPIC_API_KEY — using extractive summarization fallback")
    ...
    if use_summary:
        _api_key = os.environ.get("ANTHROPIC_API_KEY", "").strip()
        if _api_key:
            summary_backend = "Claude (Haiku)"
            spinner_msg = "Summarising with Claude…"
        else:
            summary_backend = "extractive TF-IDF"
            spinner_msg = "Summarising…"
    ...
    with tabs[1]:
        ...
        if summary_text:
            ...
            if summary_backend:
                st.caption(f"Generated by **{summary_backend}**")
            st.download_button(
                "⬇️ Download summary",
                summary_text,
